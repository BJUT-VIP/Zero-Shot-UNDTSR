...
class ZSSR:
    ...
    def __init__(self, input_img, conf=Config(), ground_truth=None, kernels=None):
        ...
        print(self.Y)
        self.model = simpleNet(self.Y, scale_factor=Config.sr)
    ...
    def forward_pass(self, lr_son, hr_father_shape=None):
        ...
        interpolated_lr_son = lr_son.copy()
        if self.Y == True:
    ...
    def learning_rate_policy(self):
        ...
        if (not (1 + self.iter) % self.conf.learning_rate_policy_check_every
            ...
            [slope, _], [[var, _], _] = np.polyfit(self.mse_steps[-(self.conf.learning_rate_slope_range //   # 1.5
                                                                    self.conf.run_test_every):],  # 50
    ...
    def train(self):
        ...
        for self.iter in range(self.conf.max_iters):
            ...
            # self.train_output = self.forward_backward_pass(self.lr_son, self.hr_father,criterion,criterion_L1,optimizer, vggmodel)
            ...
            if not self.iter % self.conf.display_every:
                print('sf:', self.sf*self.base_sf, ', iteration: ', self.iter, ', loss: {:3f}'.format(self.loss[self.iter].data))
